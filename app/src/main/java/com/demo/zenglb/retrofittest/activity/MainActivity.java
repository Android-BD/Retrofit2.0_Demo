...
import com.demo.zenglb.retrofittest.http.rxhttp.BaseObserver;
...
//		});
    ...
    private void loginByRxJava2() {
        ...
        HttpCall.getApiService(null).goLoginByRxjava2(loginParams)
    ...
    private void Login() {
        ...
        //2.Generic Programming Techniques is the basis of Android develop
        HttpCall.getApiService(null).goLogin(loginParams)
    ...
    private void refreshToken() {
        ...
        HttpCall.getApiService(null).refreshToken(loginParams)
                .enqueue(new HttpCallBack<LoginResult>(this) {
                    @Override
                    public void onSuccess(LoginResult loginResult) {
                        Log.e(TAG, loginResult.toString());
                        message.setText(loginResult.toString());  //display result
                        HttpCall.setToken("Bearer " + loginResult.getAccessToken());
                        refreshToken = loginResult.getRefreshToken();
                    }
    ...
    private void requestModules() {
        HttpCall.getApiService(null).getModules().enqueue(new HttpCallBack<Modules>(this) {
            @Override
            public void onSuccess(Modules modulesHttpResponse) {
                Log.e(TAG, modulesHttpResponse.toString());
                message.setText(modulesHttpResponse.toString());
            }
...
//	}
    ...
    private void getMessages() {
        HttpCall.getApiService(null).getMessages(1, 3)
                .enqueue(new HttpCallBack<List<Messages>>(this) {
                    @Override
                    public void onSuccess(List<Messages> listHttpResponse) {
                        Log.e(TAG, listHttpResponse.toString());
                        message.setText(listHttpResponse.toString());
                    }
